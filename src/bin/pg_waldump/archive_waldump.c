				   ...
				   char **fname)
{
	...
	/*
	 * For a correct comparison, we must remove any '.' or '..' components from
	 * the member pathname. Similar to member_verify_header(), we prepend './'
	 * to the path so that canonicalize_path() can properly resolve and strip
	 * these references from the tar member name
	 */
