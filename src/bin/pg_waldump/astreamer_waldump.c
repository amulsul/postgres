...
static int	astreamer_archive_read(XLogDumpPrivate *private);
				   ...
				   XLogDumpPrivate *private)
{
	...
	volatile StringInfo astreamer_buf = private->archive_streamer_buf;
	...
	while (nbytes > 0)
	{
		...
		XLogRecPtr	endPtr = private->archive_streamer_read_ptr;
		...
		{
			/* Fetch more data */
			if (astreamer_archive_read(private) == 0)
 ...
 */
static int
astreamer_archive_read(XLogDumpPrivate *private)
{
	int			rc;
	char	   *buffer;
	...
	rc = read(private->archive_fd, buffer, READ_CHUNK_SIZE);
	if (rc < 0)
	 ...
	 */
	if (rc > 0)
		astreamer_content(private->archive_streamer, NULL,
...
astreamer_waldump_content_new(astreamer *next, XLogRecPtr startptr,
							  XLogRecPtr endPtr, XLogDumpPrivate *private)
{
	...
	*((const astreamer_ops **) &streamer->base.bbs_ops) =
	...
	streamer->timeline = private->timeline;
	streamer->nextSegNo = streamer->startSegNo;
	streamer->private = private;
						  ...
						  astreamer_archive_context context)
{
	...
	XLogDumpPrivate *private = mystreamer->private;
		...
		case ASTREAMER_MEMBER_HEADER:
			{
				...
				if (private->archive_streamer_buf->len != 0)
				{
					XLogRecPtr	recPtr;

					XLogSegNoOffsetToRecPtr(segNo, 0, WalSegSz, recPtr);
					Assert(private->archive_streamer_read_ptr == recPtr);
				}
#endif
				...
				XLogSegNoOffsetToRecPtr(segNo, 0, WalSegSz,
										private->archive_streamer_read_ptr);
			...
			if (mystreamer->writeThisSeg)
			{
				...
				if (len > 0 && fwrite(data, len, 1, mystreamer->segFp) != 1)
				{
					...
					pg_fatal("could not write to file \"%s/%s\": %m",
							 private->tmpdir, fname);
				}
				break;
			}

			/* Or, copy contents to buffer */
			private->archive_streamer_read_ptr += len;
