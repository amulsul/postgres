							  ...
							  Oid ReferencingParentUpdTrigger)
{
		...
		currcon->condeferred != cmdcon->initdeferred)
	{
		...
		/*
		 * Note that even if deferrability is requested to be altered along
		 * with enforceability, we don't need to call
		 * AlterConstrTriggerDeferrability() explicitly. Changing
		 * enforceability involves either creating or dropping the trigger,
		 * during which the deferrability will be set accordingly.
		 */
		if (cmdcon->alterEnforceability)
		{
